...
Модуль для Bun runtime (совместим с Node.js), который упрощает интеграцию ежемесячных подписок Tribute (Telegram) в любые сервисы и биллинги. Менеджер оформляет подписку по заранее подготовленным ссылкам, проверяет подпись вебхуков Tribute и синхронизирует статусы с вашим хранилищем.
...
- понимает «сырые» тела запросов как `Buffer`, `Uint8Array`, `ArrayBuffer` — удобно подключать напрямую к `Bun.serve()` или совместимым fetch-серверам;
...
```bash
bun add tribute-nodejs-api
```

> Требуется Bun версии 1.0 и выше; библиотека совместима с Bun runtime и может использоваться в приложениях на Bun.
...
const manager = new TributeSubscriptionManager(
  createTributeConfig({
    ...
    apiKey: Bun.env.TRIBUTE_API_KEY,
...
const server = Bun.serve({
  ...
  async fetch(request) {
    ...
    if (request.method === 'POST' && url.pathname === '/webhooks/tribute') {
      const rawBody = await request.arrayBuffer();
      const signature = request.headers.get('trbt-signature');
      try {
        await manager.handleWebhook(rawBody, signature);
...
console.log(`Tribute webhooks listening on http://localhost:${server.port}`);
```
